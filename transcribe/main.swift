...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func loadModel() {
        ...
        guard FileManager.default.fileExists(atPath: modelDir) else {
            print("[VoxtralDictate] ❌ Model directory not found: \(modelDir)")
            print("  → Download: cd ~/voxtral.c && ./download_model.sh")
            print("  → Or set VOXTRAL_MODEL_DIR environment variable")
            DispatchQueue.main.async {
                let alert = NSAlert()
                alert.messageText = "Model Not Found"
                alert.informativeText = "Voxtral model not found at:\n\(modelDir)\n\nRun: cd ~/voxtral.c && ./download_model.sh"
