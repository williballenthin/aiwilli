...
- entry types: `transcript` (voice), `handwriting` (reMarkable), `meeting notes` (calendar docs), `meeting chat` (calendar chats), `todo`.
...
- summary is generated by LLM (same model as transcription) with a prompt requesting three sentences for a daily index overview.
