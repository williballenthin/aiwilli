...
CONFIG_DIR = Path(os.environ.get('XDG_CONFIG_HOME', Path.home() / '.config')) / 'wballenthin' / 'weave'
...
def main(source: str):
    ...
    for event in events:
        ...
        doc_attachments = [a for a in all_attachments if a.get('mimeType') == 'application/vnd.google-apps.document']
        chat_attachments = [a for a in all_attachments if a.get('mimeType') == 'text/plain' and a.get('title', '').endswith('- Chat')]
        ...
        for att in doc_attachments:
            ...
            if is_shared_notes(att_title):
                section = extract_section_for_date(md.decode('utf-8', errors='replace'), start_dt.date())
                if section is None:
            ...
            front_matter = build_front_matter(source, event_name, start_dt, doc_url, event_url, attendees, 'meeting_notes', attended)
        ...
        for att in chat_attachments:
            ...
            front_matter = build_front_matter(source, event_name, start_dt, doc_url, event_url, attendees, 'meeting_chat', attended)
