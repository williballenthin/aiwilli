...
class TestIssueService:
    ...
    def test_deleted_ids_not_reused(self, taskwarrior_env: dict[str, str]) -> None:
        """Creating new issues after deletion should not reuse deleted IDs."""
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
        ...
        # New epic should be TEST-4, not TEST-3
    ...
    def test_deleted_child_ids_not_reused(
        ...
        """Creating new children after deletion should not reuse deleted IDs."""
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
...
class TestUpdateIssue:
    def test_update_title(self, taskwarrior_env: dict[str, str]) -> None:
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
    ...
    def test_update_body(self, taskwarrior_env: dict[str, str]) -> None:
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
        ...
        issue = service.get_issue(tw_id)
        assert issue.tw_body == "New body"

    def test_update_title_and_body(self, taskwarrior_env: dict[str, str]) -> None:
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
    ...
    def test_update_extracts_references(self, taskwarrior_env: dict[str, str]) -> None:
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
        ...
        assert "TEST-2" in issue.tw_refs

    def test_update_empty_body_clears_refs(self, taskwarrior_env: dict[str, str]) -> None:
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
    ...
    def test_update_nonexistent_issue(self, taskwarrior_env: dict[str, str]) -> None:
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
...
class TestGetIssueTree:
    def test_tree_includes_orphan_story(self, taskwarrior_env: dict[str, str]) -> None:
        ...
        ids = [i.tw_id for i in tree]
        assert "TEST-1" in ids

    def test_tree_includes_orphan_task(self, taskwarrior_env: dict[str, str]) -> None:
...
class TestGetIssueWithContext:
    def test_returns_ancestors_chain(self, taskwarrior_env: dict[str, str]) -> None:
        ...
        issue, ancestors, siblings, descendants, referenced, referencing = service.get_issue_with_context(
            "TEST-1-1a"
        )

        assert issue.tw_id == "TEST-1-1a"
        assert len(ancestors) == 2
        assert ancestors[0].tw_id == "TEST-1-1"  # immediate parent
        assert ancestors[1].tw_id == "TEST-1"  # grandparent (root)

    def test_returns_siblings(self, taskwarrior_env: dict[str, str]) -> None:
        ...
        assert issue.tw_id == "TEST-1-2"
        assert len(siblings) == 2
        sibling_ids = [s.tw_id for s in siblings]
    ...
    def test_returns_descendants_recursively(
        ...
        assert issue.tw_id == "TEST-1"
        assert len(descendants) == 4
        desc_ids = [d.tw_id for d in descendants]
        assert desc_ids == ["TEST-1-1", "TEST-1-1a", "TEST-1-1b", "TEST-1-2"]

    def test_orphan_has_empty_ancestors(self, taskwarrior_env: dict[str, str]) -> None:
    ...
    def test_leaf_has_empty_descendants(self, taskwarrior_env: dict[str, str]) -> None:
    ...
    def test_root_orphans_have_no_siblings(self, taskwarrior_env: dict[str, str]) -> None:
    ...
    def test_nonexistent_issue_raises(self, taskwarrior_env: dict[str, str]) -> None:
    ...
    def test_returns_referenced_issues(self, taskwarrior_env: dict[str, str]) -> None:
        ...
        assert issue.tw_id == "TEST-3"
        assert len(referenced) == 2
        ref_ids = [r.tw_id for r in referenced]
        assert "TEST-1" in ref_ids
        assert "TEST-2" in ref_ids

    def test_returns_referencing_issues(self, taskwarrior_env: dict[str, str]) -> None:
        ...
        ref_ids = [r.tw_id for r in referencing]
        assert "TEST-2" in ref_ids
        assert "TEST-3" in ref_ids

    def test_referenced_and_referencing_sorted_by_id(self, taskwarrior_env: dict[str, str]) -> None:
...
class TestBacklogIssues:
    ...
    def test_backlog_rejects_unblock(self, taskwarrior_env: dict[str, str]) -> None:
        backend = TaskWarriorBackend(env=taskwarrior_env)
        service = IssueService(backend, project="test", prefix="TEST")
